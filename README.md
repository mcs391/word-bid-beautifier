...
> **ryusuke-bid-beautifier** — 专为ryusuke格式投标技术文件设计的自动化排版优化工具
...
| **Phase 2** | 样式美化 | 智能识别并套用 ryusuke 标题样式、清除重复编号、添加首行缩进、优化缩进值和字体间距，并统一表格正文样式 |

### 详细功能清单

#### 📐 Phase 0 — 根本性编号修复（三层面统一）★V2★
> **核心价值**：修复后，**手动套用 ryusuke标题2/3 样式时 Word 会自动继承正确的章节号计数**
...
#### 🎨 Phase 2 — 样式美化
- **样式套用**：智能识别四级/五级标题，自动套用 `ryusuke标题4` / `ryusuke标题5` 样式
...
- **间距优化**：调整标题段前段后间距和行距
- **表格正文接管**：自动识别 `hik-表格正文` 并重命名/固化为 `ryusuke-表格正文`
...
### 样式 ID 对照表（ryusuke 格式）

| 层级 | style ID | 样式名称 | 编号格式 |
|:----:|:--------:|----------|----------|
| 章标题 | 12 | ryusuke标题1 | 第%1章 |
| 二级标题 (H2) | 13 | ryusuke标题2 | %1.%2. |
| 三级标题 (H3) | 14 | ryusuke标题3 | %1.%2.%3. |
| 四级标题 (H4) | 15 | ryusuke标题4 | %1.%2.%3.%4. |
| 五级标题 (H5) | 16 | ryusuke标题5 | %1.%2.%3.%4.%5. |
| 正文 | 17 | ryusuke正文 | 无编号 |
| 表格正文 | 37 | ryusuke-表格正文 | 居中、小四、宋体 |
...
**Phase 0 V2 的核心逻辑**：很多投标文档存在**编号分裂问题**——不同标题样式指向不同的 abstractNum（多级列表定义），导致计数器不同步。例如手动在第14章套用 ryusuke标题2 样式时，编号显示为 9.x 而非 14.x。V2 通过三层面修复彻底解决：
...
2. **仅适用于 ryusuke格式**：样式 ID (12–16) 及扩展样式如 `ryusuke-表格正文` 是针对 ryusuke 投标模板沉淀的，其他格式需先确认样式映射
