...
| 阶段 | 名称 | 功能说明 |
|:----:|------|---------|
| **Phase 0** | H2/H3 编号跟随修复 ★NEW | 为章节/H2/H3 标题插入自动编号引用，解决「第10章后编号仍显示9.6」等章节号断裂问题 |
...
- 自动为**章标题 / 二级标题 / 三级标题**段落插入 `<w:numPr>` 编号引用
- 让 Word 多级列表自动按章节生成正确编号（`10.1`、`10.1.1` …）
- 清除文本中残留的旧编号前缀，避免重复显示
- 支持任意数量的章节，无需手动配置
...
**Phase 0 的核心逻辑**：当段落的 `<w:pPr>` 中缺少 `<w:numPr>` 时，Word 不知道该段落属于多级列表的哪一层级，因此无法自动编号。本工具会为所有 H1/H2/H3 段落补上正确的 numPr 引用。
